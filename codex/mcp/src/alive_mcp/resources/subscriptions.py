...
* **The filesystem side** -- a portable polling observer running a single
  recursive watch over the resolved World root. Its handler runs OFF the
  asyncio event loop and must marshal work back
...
Design choices
--------------
**Single polling observer for server lifetime.** FastMCP v1.27 does not expose
...
lifetime sidesteps leak-prone lazy-start/stop logic. A 500ms polling
interval avoids native watcher crashes observed in sandboxed macOS/Python
runtimes. Its filesystem cost on large Worlds remains a private-alpha gap.
...
def start_observer(
    ...
    *,
    ...
    observer: BaseObserver = Observer(timeout=0.5)
