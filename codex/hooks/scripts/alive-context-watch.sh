...
[ -z "$SESSION_ID" ] && exit 0
SAFE_SESSION_ID=$(printf '%s' "$SESSION_ID" | tr -cd 'A-Za-z0-9._-' | cut -c1-96)
...
if [ -f "$CTX_FILE" ]; then
  ...
  if [ -n "$CTX_PCT" ] && [ "$CTX_PCT" -gt 0 ] 2>/dev/null; then
    ...
    for THRESHOLD in 80 60 40 20; do
      MARKER="/tmp/alive-ctx-${SAFE_SESSION_ID}-${THRESHOLD}"
    ...
    if [ -n "$FIRE_THRESHOLD" ]; then
      ...
      for T in 20 40 60 80; do
        if [ "$T" -le "$FIRE_THRESHOLD" ]; then
          touch "/tmp/alive-ctx-${SAFE_SESSION_ID}-${T}"
        fi
      done
      THRESHOLD="$FIRE_THRESHOLD"

        # Build bounded refresh content based on threshold level.
...
- Use the world and search skills for specific local retrieval.
</ALIVE_REFRESH>"
        else
          # Detailed refresh remains bounded; it does not read world indexes.
          REFRESH="<ALIVE_REFRESH threshold=\"${THRESHOLD}%\">
Context is at ${CTX_PCT}%. Bounded context refresh:
- Stash decisions, tasks, and notes. Surface on change.
- Verify past context via subagent before asserting. Never guess from memory.
- Bundle awareness: deliverable or future audience = bundle.
- Read before speaking. Never answer from memory about file contents.
- Use explicit world/search requests for detailed retrieval.
</ALIVE_REFRESH>"
        fi
...
${ACTIVE_STASHES}
        ...
        REFRESH=$(compose_bounded_orientation_context "$REFRESH" "$ORIENTATION")
