...
>
> Use the apply_patch tool to update the file. The scaffolded file already exists.
...
>
> Use the apply_patch tool to update the file.
...
The log guardian hook blocks the `apply_patch` tool on any file named `log.md`. This is by design — logs are append-only. But during migration of a brand-new walnut, the log.md was already created by `$alive-create-walnut` Step 6 with a single "Walnut created" entry. The migration needs to replace it with richer history.

**Use bash (`cat >`) to write log.md.** The log guardian hook applies to the apply_patch tool, not bash.
...
5. **Log guardian hook.** The apply_patch tool is blocked on `log.md` by the log guardian hook. Use bash (`cat >`) for writing log.md during migration. This only applies to new walnut creation — normal log updates during sessions use apply_patch to prepend.
