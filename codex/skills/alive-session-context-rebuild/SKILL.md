...
## Rebuild Flow — Numbered Preconditions

Run each phase in order. Each phase has a defined output that the next phase consumes.

### Phase 1 — Scout (single agent)

Precondition: a walnut name has been resolved.

1. Dispatch one subagent to map the territory
2. The scout calls `$alive-session-history` to get the lay of the land
3. The scout reports back:
...
4. Present scout summary to the human and ask for rebuild scope
...
Precondition: Phase 1 complete and rebuild scope confirmed.

1. Dispatch parallel subagents based on scope (one per session, or one per domain)
2. Each agent gets specific file paths and session IDs — no broad searches
3. Wait for all agents to return before moving to Phase 3
...
- **Task agent** — uses `tasks.py list` to query task state. Returns what is done, what is in progress, what is blocked.
...
- **Conflict agent** (only if overlapping sessions detected in Phase 1) — reads both sessions' log entries and file changes side by side. Identifies contradictions: different decisions on the same topic, competing task states, divergent draft versions.

Each agent returns structured findings. They do not summarise — they return the actual context with file paths, timestamps, and quotes.

### Phase 3 — Merge & Present

Precondition: Phase 2 complete and all agents have returned.

1. Combine all agent findings into a single reconstructed context
2. Highlight any conflicts that need resolution
3. Present to the human in a single bordered block
...
Precondition: Phase 3 complete and the human has confirmed.

1. The agent now holds full reconstructed context
2. Unresolved conflicts are stashed as decisions-needed
3. Any conflict resolutions are logged as decisions
4. Work continues normally — stash, save, the whole flow
...
Do not over-swarm. Scale agents to the rebuild scope:
