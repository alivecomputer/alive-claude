...
Five parallel agents reconstruct a session transcript. Each extracts one dimension. The goal is full awareness transplant — after absorbing all five outputs, the current agent has the same level of awareness as the one that ran the original session.
...
summarise — it is to RECONSTRUCT. The goal is that the agent reading your output
absorbs enough of the original session's substance that it has the same level of
awareness as the agent that ran it.
...
2. **Flow states** — identify moments where the human and agent were locked in
...
6. **The dynamic** — how did the human and agent relate in this session?
   Was it directive (human leading, agent executing)? Collaborative (thinking
   together)? Was the agent proactive or reactive? Did the dynamic shift?
...
5. **Why NOT the alternatives** — what was wrong with or less ideal about the other
   options
6. **Who decided** — did the human decide, did the agent recommend and the
...
2. **Agent quotes** — things the agent said that the human responded well to:
...
3. **Exchange moments** — back-and-forth sequences (2-4 messages) where something
   clicked. The human said X, the agent said Y, the human said "yes
...
2. **Open questions** — questions that were raised during the session but never
   ...
   agent. Note the context — why was this question raised?

3. **Promised but not delivered** — things the agent said it would do, or the
...
```
╭─ revive — narrative & energy
│  [agent 1 output]
╰─

╭─ revive — decisions & reasoning
│  [agent 2 output]
╰─

╭─ revive — quotes & moments
│  [agent 3 output]
╰─

╭─ revive — technical substance
│  [agent 4 output]
╰─
