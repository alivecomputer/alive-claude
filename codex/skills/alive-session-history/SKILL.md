...
Not a search (that's find — searches content across walnuts). History searches SESSIONS — what agents did, what was discussed, what was decided. For deep context extraction from source material, use `$alive-mine-for-context`.
...
Structured, fast, indexed. Session ID, walnut, model, timestamps, stash items, transcript path. Every session leaves one of these.
...
For tool_use entries, note what tools were called (Read, apply_patch, Bash, Grep, Glob,
...
[1-2 paragraphs. Write this for an agent with ZERO memory of this session. What was the
...
When the human selects heavy revive, read `heavy-revive.md` from this skill's directory (same folder as this file). That file contains the full agent prompts and dispatch instructions. Do NOT attempt to run heavy revive without reading that file first.
...
Generate a context briefing that a new session can load to continue the work. This is the output — a structured document that captures:
...
2. **Fallback:** Scan `~/.claude/projects/*/` for `{session_id}*.jsonl` — Codex names transcripts using the session UUID
...
|----------|----------------|--------|
| Codex | `~/.claude/projects/<hash>/<session>.jsonl` | JSONL (messages + tool calls) |
...
**Privacy note:** Transcripts live on the human's machine in Codex's project directory. They never leave. History reads them locally.
