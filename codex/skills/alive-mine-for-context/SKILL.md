...
> ⚠️ Currently limited in Codex — external URL fetching requires MCP tool (coming in v0.5). Use local file sources only.
...
This means the agent can resume mining across sessions — it knows exactly what has been done and what remains.
...
The agent resolves transcript paths using the same discovery mechanism as history (see `$alive-session-history` Transcript Discovery section).
