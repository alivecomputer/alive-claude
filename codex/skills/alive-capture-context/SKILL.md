...
- The agent detects external content and offers to capture
...
1. Write raw file → `{bundle}/raw/[name].[ext]` via `apply_patch`
2. Update bundle `context.manifest.yaml` → add entry to `sources:` list in frontmatter via `apply_patch`
...
The summary should be **detailed enough that you rarely need the raw file.** Compose it like someone who has 30 seconds to understand what this reference contains.
...
Offer deep capture for content that is clearly rich:
...
**This is critical.** When the agent does significant research during a session — web searches, code analysis, system exploration, competitor analysis, architecture research, API investigation — that knowledge MUST NOT die with the conversation. It cost tokens, time, and thinking to produce. It is a first-class reference.

The agent should proactively offer to capture when:
...
- You asked the agent to investigate something
- The agent produced a synthesis, comparison, or analysis worth keeping
...
- Obvious facts the agent already knew — don't create a file for common knowledge
- Research that led nowhere — unless the dead end is itself useful ("we looked into X, it doesn't work because Y")

The test: **would the next session waste time rediscovering this?** If yes, capture it. If no, let it go.
