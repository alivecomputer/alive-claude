...
Default: single-walnut focus. People involved are loaded automatically (frontmatter only) — you cannot work on a venture without knowing who is in it.
...
## Load Sequence — Numbered Preconditions

Run each step in order. Tier 1 is mandatory. Tier 2 is automatic. Tier 3 is offered.

### Step 1 — Tier 1 Brief Pack (3 files)

Precondition: a walnut name has been resolved.

1. Read `_kernel/key.md` — full file (identity, people, links, rhythm)
2. Read `_kernel/now.json` — full file (phase, next action, bundle statuses with task summaries, recent sessions, nested walnut state, blockers, context paragraph)
3. Read `_kernel/insights.md` — frontmatter only (what domain knowledge sections exist)
4. Emit one `>` line per file read

**DO NOT read any other files at this stage.** No log.md. No bundle manifests. No tasks files. No squirrel entries. All of that data is already in now.json — the projection script aggregated it. Reading source files at load wastes context window on data already loaded.

### Step 2 — Inbox triage (background, non-blocking)

Precondition: Step 1 complete.

1. Check if `03_Inbox/` has items: `ls 03_Inbox/ 2>/dev/null`
2. If yes, dispatch a background triage agent (same spec as in the world skill — reads items, tags type/destination/priority, returns structured report)
3. Do NOT wait for it — continue to Step 3 immediately. Results arrive while later steps run.
...
**Backward compat fallback chain for now.json:**
...
### Step 3 — Tier 2 People Context (automatic)

Precondition: Step 1 complete. Runs automatically — do not ask.

1. From key.md `people:`, resolve each name to a person walnut at `02_Life/people/{kebab-case-name}/`
2. For each person, read `_kernel/key.md` frontmatter only (name, role, tags, last updated, rhythm)
3. Emit one `>` line per person read

Lightweight (3-5 small reads) and always happens.
...
### Step 4 — Tier 3 Bundle Deep-Load (on demand)

Precondition: Step 1 complete and `now.json` has a `bundle:` field pointing to an active bundle.

1. Show the bundle summary (already known from Step 1)
2. Offer deep-load:
...
**If deep load selected:**

1. Read `{name}/context.manifest.yaml` — full file (context, changelog, work log, session history)
2. Run `tasks.py list --walnut {path} --bundle {name}` for the detailed task view (do NOT read `tasks.json` directly)
3. Use `apply_patch` to write an `active_sessions:` entry to the bundle's `context.manifest.yaml` — claim this session so other agents know you are here
...
### Step 5 — Spotted Observation

Precondition: Steps 1-3 complete.

1. The brief pack gives you everything: phase, bundles, tasks, blockers, recent sessions, nested walnuts.
2. Find ONE observation worth noticing — a blocker that has been sitting, a bundle with no recent sessions, a next action that is overdue, a pattern across task counts.
3. Render it as a single bordered block.
...
If there is not enough context for a genuine observation, skip it. An obvious one is worse than none.

### Step 6 — Bundle Prompt

Precondition: Step 5 complete.

Prompt with bundle awareness:
...
"Just chat" — freestyle, the agent loads more later if needed.
...
This is rare. Most cross-walnut context comes naturally from the people tier (Step 3) — loading a venture automatically gives you lightweight context on everyone involved.
