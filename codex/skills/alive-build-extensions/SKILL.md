...
The agent determines: is this a skill (process), a rule (constraint), a hook (automation), or a combination?

```
╭─ that sounds like a hook — an automated trigger that fires on save.
│  Want me to build it?
│
│  It would:
...
Codex only discovers skills in `~/.claude/skills/`. After writing a custom skill to `.alive/skills/`, **always create the symlink**:
...
**This is mandatory.** A skill without its symlink is invisible to Codex.
...
The agent watches for repeated patterns across sessions. When it spots the human doing the same thing manually:
...
```
╭─ this skill could work for other builders
